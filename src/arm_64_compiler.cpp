...
#ifndef ARM_64_COMPILER_SOURCE
#define ARM_64_COMPILER_SOURCE
...
void Command::evaluate_expression(std::ostream & file)
{
  ...
  for (; eval.size();) {
    ...
    if (aExpType == VAR)  {
      ...
      file<<"\tmov %x1, $"<<a.int_arg<<std::endl;
      file<<"\tpush {%x1}"<<std::endl;
      stack_depth += 4;
      ...
      m_exp_literals.pop();
      file<<"\tldr %x1, =S"<<idx<<std::endl;
      file<<"\tpush {%x1}"<<std::endl;
      stack_depth += 4;
      continue;
    } else if (aExpType == PTRDEREF) {
      file<<"\tpop {%x1}"<<std::endl;
      file<<"\tldr %x1, [%x1]"<<std::endl;
      file<<"\tpush {%x1}"<<std::endl;
      continue;
    } else if (aExpType == LOGNOT) {
      file<<"\tpop %{r1}"<<std::endl;
      file<<"\tcmp %x1, $0"<<std::endl;
      file<<"\tmoveq %x0, $0"<<std::endl;
      file<<"\tmovneq %x0, $1"<<std::endl;
      file<<"\tpush {%x0}"<<std::endl;
      ++nszcount;
      continue;
    }
    
    if (stack_depth >= 8) {
      file<<"\tpop {%x1, %x2}"<<std::endl;
      stack_depth -= 8;
    } else continue;
    switch (type) {
    case ADD:
       ...
       */
      file<<"\tadd %x0, %x1, %x2"<<std::endl;
      goto do_default;
    case SUB:
      file<<"\tsub %x0, %x2, %x1"<<std::endl;
      goto do_default;
    case MUL:
      file<<"\tmul %x0, %x1, %x2"<<std::endl;
      goto do_default;
    case DIV:
      /* @todo case divide by zero */
      // Do we need to move 0 into r0? commenting
      // out for now.
      file<<"\teor %x0, %x0, %x0"<<std::endl;
      file<<"\tmov %x5, $1"<<std::endl;
      file<<"\tmov %x3, #-1"<<std::endl;
      file<<"\tcmp %x1, $0"<<std::endl;
      file<<"\tmulle %x5, %x5, %x3"<<std::endl;
      file<<"\tmulle %x1, %x1, %x3"<<std::endl;
      file<<"\tcmp %x2, $0"<<std::endl;
      file<<"\tmulle %x5, %x3, %x5"<<std::endl;
      file<<"\tmulle %x2, %x3, %x2"<<std::endl;
      file<<"DIV"<<divcount++<<":\tcmp %x1, %x2"<<std::endl;
      file<<"\tbgt DDIV"<<divcount-1<<std::endl;
      file<<"\tsub %x2, %x2, %x1"<<std::endl;
      file<<"\tadd %x0, $1"<<std::endl;
      file<<"\tb DIV"<<divcount-1<<std::endl;
      file<<"DDIV"<<divcount-1<<":";
      file<<"\tmul %x0, %x0, %x5"<<std::endl;
      goto do_default;
    case MOD:
      file<<"\tudiv %x0, %x2, %x1"<<std::endl;
      file<<"\tmls %x2, %x1, %x0, %x2"<<std::endl;
      goto do_default;
    case GT:
      t1 = arg_types.top(); arg_types.pop();
      t2 = arg_types.top(); arg_types.pop();
      if (t1 == STRNG && t2 == STRNG) {
	file<<"\tmov %x0, %x1"<<std::endl;
	file<<"\tmov %x1, %x2"<<std::endl;
	file<<"\tbl strcmp"<<std::endl;
	file<<"\tcmp %x0, $0"<<std::endl;
	file<<"\tmovgt %x0, $1"<<std::endl;
	file<<"\tmovle %x0, $0"<<std::endl;
      } else {
	file<<"\tcmp %x2, %x1"<<std::endl;
	file<<"\tmovgt %x0, $1"<<std::endl;
	file<<"\tmovle %x0, $0"<<std::endl;
      }
      nszcount++;
	...
	file<<"\tbl strcmp"<<std::endl;
	file<<"\tcmp %x0, $0"<<std::endl;
	file<<"\tmovge %x0, $1"<<std::endl;
	file<<"\tmovlt %x0, $0"<<std::endl;
      } else {
	file<<"\tcmp %x2, %x1"<<std::endl;
	file<<"\tmovge %x0, $1"<<std::endl;
	file<<"\tmovle %x0, $0"<<std::endl;
	...
	file<<"\tbl strcmp"<<std::endl;
	file<<"\tcmp %x0, $0"<<std::endl;
	file<<"\tmovlt %x0, $1"<<std::endl;
	file<<"\tmovge %x0, $0"<<std::endl;
      } else {
	file<<"\tcmp %x2, %x1"<<std::endl;
	file<<"\tmovlt %x0, $1"<<std::endl;
	file<<"\tmovge %x0, $0"<<std::endl;
	...
	file<<"\tbl strcmp"<<std::endl;
	file<<"\tcmp %x0, $0"<<std::endl;
	file<<"\tmovle %x0, $1"<<std::endl;
	file<<"\tmovgt %x0, $0"<<std::endl;
      } else {
	file<<"\tcmp %x1, %x2"<<std::endl;
	file<<"\tmovle %x0, $1"<<std::endl;
	file<<"\tmovgt %x0, $0"<<std::endl;
      }
      nszcount++;
	...
	file<<"\tbl strcmp"<<std::endl;
	file<<"\tcmp %x0, $0"<<std::endl;
	file<<"\tmoveq %x0, $1"<<std::endl;
	file<<"\tmovne %x0, $0"<<std::endl;
      } else {
	file<<"\tcmp %x1, %x2"<<std::endl;
	file<<"\tmoveq %x0, $1"<<std::endl;
	file<<"\tmovne %x0, $0"<<std::endl;
	...
	file<<"\tmoveq %x0 $0"<<std::endl;
      ...
      goto do_default;
    case LOGAND:
      file<<"\tmov %x0, $1"<<std::endl;
      file<<"\tcmp %x1, $0"<<std::endl;
      file<<"\tbeq AND"<<nszcount<<std::endl;
      file<<"\tcmp %x2, $0"<<std::endl;
      file<<"\tbeq AND"<<nszcount<<std::endl;
      file<<"\tb AND"<<nszcount+1<<std::endl;
      file<<"AND"<<nszcount<<":\tmov %x0, $0"<<std::endl;
      file<<"AND"<<nszcount+1<<":"<<std::endl;
      nszcount += 2;
      goto do_default;
    case LOGOR:
      file<<"\tadd %x1, %x1, %x2"<<std::endl;
      file<<"\tmov %x0, $1"<<std::endl;
      file<<"\tcmp %x1, $0"<<std::endl;
      file<<"\tbne NSZ"<<nszcount<<std::endl;
      file<<"\tmov %x0, $0"<<std::endl;
      file<<"NSZ"<<nszcount<<": @ label used to set as true"<<std::endl;
      nszcount++;
      goto do_default;
    case LOGXOR:
      file<<"\tmul %x3, %x1, %x2"<<std::endl;
      file<<"\tadd %x4, %x1, %x2"<<std::endl;
      file<<"\tmov %x1, $1"<<std::endl;
      file<<"\tmov %x2, $1"<<std::endl;
      file<<"\tcmp %x3, $0"<<std::endl;
      file<<"\tbne NSZ"<<nszcount<<std::endl;
      file<<"\tmov %x1, $0"<<std::endl;
      file<<"NSZ"<<nszcount++<<": @ r1 <-- a && b"<<std::endl;
      file<<"\tcmp %x4, $0"<<std::endl;
      file<<"\tbne NSZ"<<nszcount<<std::endl;
      file<<"\tmov %x2, $0"<<nszcount<<std::endl;
      file<<"NSZ"<<nszcount<<": @ r1 <-- a || b"<<std::endl;
      file<<"\teor %x0, %x1, %x2"<<std::endl;
      nszcount++;
      goto do_default;
    default:
			    ...
			    std::ostream & file)
{
  ...
  for (int x = 0; x < m_int_declarations.size(); ++x) {
    if (m_int_declarations[x] == varname) {
      file<<"\tmov %x1, %x9"<<std::endl;
      file<<"\tldr %x1, [%x9, #"<<8 * x<<"]"<<std::endl;
      file<<"\tldr %x1, [%x1]"<<std::endl;
      file<<"\tpush {%x1}"<<std::endl;
      vartype.push(INTGR);
      return;
...
void Command::doMain(std::ostream & file)
{
  ...
  for (int x = 0, intassdex = 0, stringdex = 0, exprdex = 0,
    ...
    if (m_execOrder[x] == cmd_type::READ_STRING) {
      file<<"\tldr %x0, =string_fmt"<<std::endl;
      file<<"\tldr %x1, =IS"<<stringdex<<std::endl;
      ...
      file<<"\tldr %x0, =IS"<<stringdex<<std::endl;
      file<<"\tbl gets"<<std::endl;
      file<<std::endl;
      
      ++stringdex;
    } else if (m_execOrder[x] == cmd_type::DECL_INT) {
      file<<"\tmov %x0, $8"<<std::endl;
      file<<"\tbl malloc"<<std::endl;
      file<<"\tstr %x0, [%x9, #"<<sints * 8<<"]"<<std::endl;
      ...
      file<<"\tpop {%x0}"<<std::endl;
      file<<"IF"<<ifndex<<":\tcmp %x0, $0"<<std::endl;
	...
	file<<"FOR"<<forndex<<":";
	file<<"\tmov %x8, %x9"<<std::endl;
	file<<"\tldr %x8, [%x8, #"<<4 * z<<"]"<<std::endl;
	file<<"\tldr %x8, [%x8]"<<std::endl;
      } else if (y < m_int_vars.size()) {
	file<<"FOR"<<forndex<<":";
	file<<"\tmov %x8, =I"<<y<<std::endl;
	file<<"\tldr %x8, [%x8]"<<std::endl;
      } else {
	...
	exit(7);
      ...
      file<<"\tldr %x0, =num_fmt"<<std::endl;
      file<<"\tldr %x1, =I"<<intdex<<std::endl;
      file<<"\tbl scanf"<<std::endl;
      file<<std::endl;
	...
	file<<"\tldr %x1, =locals"<<std::endl;
	file<<"\tldr %x1, [%x1, #"<<4 * z<<"]"<<std::endl;
	file<<"\tldr %x1, [%x1]"<<std::endl;
      } else {
	file<<"\tldr %x1, =I"<<y<<std::endl;
	file<<"\tldr %x1, [%x1]"<<std::endl;
      } file<<"\tbl printf"<<std::endl;
      ++pintdex;
      ...
      if (!on_stack) {
       ...
       for (x = 0; x < m_int_vars.size(); ++x) {
        if (m_int_vars[x] == int_gets) break;
	     ...
	     file<<"\tldr %x3, =I"<<x<<std::endl;
      } else {
	     file<<"\tmov %x3, %x9"<<std::endl;
	     file<<"\tldr %x3, [%x3, #"<<*last_z * 4<<"]"<<std::endl;
	     delete last_z;
      }
      file<<"\tstr %x0, [%x3]"<<std::endl;
...
void Command::writeAssembly()
{
  ...
  for (int x = 0; x < m_int_declarations.size(); ++x) {
    file<<"\tldr %x0, =locals"<<std::endl;
    file<<"\tldr %x0, [%x0, #"<<8 * x<<"]"<<std::endl;
  ...
  if (!m_is_c_callable) {
    ...
    if (m_current_returns.size() > 3) {
       ...
       */
      if (m_current_returns.size()) file<<"\tldr %x9, =locals"<<std::endl;
	...
	for (y = 0; y < m_int_vars.size(); ++y) {
	    ...
	    file<<"\tmov %x6, %x9"<<std::endl;
	    file<<"\tldr %x6, [%x9, #"<<4 * z<<"]"<<std::endl;
	    file<<"\tldr %x6, [%x6]"<<std::endl;
	  } else {
	    file<<"\tldr %x6, =I"<<y<<std::endl;
	    file<<"\tldr %x6, [%x6]"<<std::endl;
	  }
	  file<<"\tmov %x"<<x<<", %x6"<<std::endl;
      }
    }
